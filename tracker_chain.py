...
def count_online_chain(game_id, game_name, limit=50):
    """链式翻页: 从 dw_offset=0 开始，跟 next_page 走到底，返回 (去重人数, 玩家详情列表)"""
    ...
    while current_path and page < 1000:
        ...
        if page <= 5:
            m = re.search(r'dw_offset=(\d+)', current_path)
            cur_off = m.group(1) if m else '?'
            log(f'    [{game_name}] 第{page:>3}页 dw_offset={cur_off:>5} +{new}({len(lst)}条) ={len(all_players)}')
        elif page % 5 == 0:
    ...
    return len(all_players), list(all_players.values())
...
def fetch_all(game_list):
    ...
    for i, (gname, gid) in enumerate(game_list):
        ...
        total, players = count_online_chain(gid, gname)
        ...
        if total is not None:
            ...
            # 保存本次扫描的玩家详情
            log_file = LOG_DIR / f'{scan_id}_{gname}.json'
            ...
            log(f'  OK {gname}: {total:,} 人 ({elapsed:.1f}s) 日志: {log_file.name}')
            log(f'  ── 状态分布 ──')
